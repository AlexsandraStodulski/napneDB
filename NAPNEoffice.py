...
def formAtendimento():
    return render_template('formPlanoIndividual.html')
...
def gravaraluno():
    ...
    aluno.cpf=request.values.get('cpf')
    bd.session.add(aluno)
    bd.session.commit()
    return 'O ALUNO FOI CADASTRADO COM SUCESSO! <BR> <a href="/formAluno">Novo</a><BR/><a href="/listarAluno">Ir para Lista</a>'

@app.route('/gravaratendimento')
def gravaratendimento():
    atendimento = Atendimento()
    atendimento.data = request.values.get('data')
    atendimento.relato_atendimento=request.values.get('relato_atendimento')
    bd.session.add(atendimento)
    bd.session.commit()

    return 'O ATENDIMENTO FOI REGISTRADO COM SUCESSO!'

@app.route('/gravaraplanoindividual')
def gravarplanoindividual():
    planoindividual = Plano_Individual()
    planoindividual.deficiencia = request.values.get('deficiencia')
    bd.session.add(planoindividual)
    bd.session.commit()

    return 'O PLANO INDIVIDUAL DE ATENDIMENTO AO ESTUDANTE DO NAPNE FOI REGISTRADO COM SUCESSO!'
...
def listarAluno():
    return render_template('listarAluno.html',alunos=Aluno.query.all())

@app.route('/listarAtendimento')
def listarAtendimento():
    return render_template('listarAtendimento.html',atendimentos=Atendimento.query.all())

@app.route('/listarPlanoIndividual')
def listarPlanoIndividual():
    return render_template('listarPlanoIndividual.html',planosindividuais=Plano_Individual.query.all())
...
class Atendimento(bd.Model):
    ...
    relato_atendimento=bd.Column(bd.String(300))

class Plano_Individual(bd.Model):
    cod_plano = bd.Column('codigo_aluno', bd.Integer, primary_key=True)
    #aluno=bd.Column('codigo_Aluno', bd.Integer, bd.ForeignKey('user.codigo'), nullable=False)
    deficiencia=bd.Column(bd.String(200))
    curso=bd.Column(bd.String(100))
    ...
    atendEduc_AdaptacaoCurr=bd.Column(bd.Boolean)
    atendEduc_ProfApoio = bd.Column(bd.Boolean)
    atendEduc_InterpLibras = bd.Column(bd.Boolean)
    atendEduc_GuiaInterp = bd.Column(bd.Boolean)
    atendEduc_MaterPedagAmpliado = bd.Column(bd.Boolean)
    atendEduc_OutrosAtendimentos = bd.Column(bd.Boolean)
    atendEduc_DescrOutrosAtendimentos = bd.Column(bd.String(200))
    encaminhamentos_realizados = bd.Column(bd.String(200))
    equipe_envolvida_preenchimento_plano_individual = bd.Column(bd.String(200))
